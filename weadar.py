...
def get_location(zip, country):
    ...
    geocode_url = ("http://api.openweathermap.org/geo/1.0/zip?"
                   f"zip={zipcode},{countrycode}&appid={API_KEY}")
...
def get_weather():
    global weather_df
    location = get_location(get_zipcode('Enter Zip Code: '),
                            get_countrycode('Enter Country Code: '))
    weather_url = ("https://api.openweathermap.org/data/3.0/onecall?"
                   f"lat={location[0]}&lon={location[1]}&exclude=minutely,"
                   f"daily&appid={API_KEY}&units=imperial")
